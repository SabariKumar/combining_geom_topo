...
from data_loading.ripsnet.rips_loader import make_dataloaders, ProteinInferenceDataset
...
def make_rips_emb(pdb_dir, emb_save_dir, betti_no, trained_model_dir):
    ...
    for betti_no in betti_nos:
        rips_model = RipsNet(
            ...
            use_bias=True,
            use_sigmoid=False)
        rips_model.load_state_dict(torch.load(
            os.path.join(trained_model_dir, f'RipsNetChkpt_Betti{betti_no}.pt'))['state_dict'])
        rips_model.eval()
        with torch.no_grad():
            for batch_idx, sample in enumerate(data_loader):
                filename = os.path.join(emb_save_dir, '_'.join([sample[0][0], 'emb', f'b{betti_no}.pt']))
                emb = rips_model(sample[1])
