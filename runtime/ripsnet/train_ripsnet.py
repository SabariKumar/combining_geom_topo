# Training script for ripsnet models. Does not use lightning for
...
from data_loading.ripsnet.rips_loader import make_dataloaders
from model.ripsnet.ripsnet import RipsNet
...
def eval_rips(betti_no, model, test_loader):
...
def rips_runner(
    ...
    for betti_ in betti_no:
        ...
        rips_model, rips_result = train_rips(
            betti_no=betti_,
        ...
        eval_rips(betti_, rips_model, test_loader)
        try:
            state_dict = rips_model.state_dict()
            checkpoint = {"state_dict": state_dict, "model_results": rips_result}
            torch.save(
                checkpoint,
                os.path.join(model_save_dir, f"RipsNetChkpt_Betti{betti_}.pt"),
            )

            if test_set_inf:
                # inf_model = rips_model.load_from_checkpoint(os.path.join(model_save_dir, f'RipsNetChkpt_Betti{betti_no}.pt'))
                rips_model.eval()

                with torch.no_grad():
                    for sample in test_loader:
                        preds = rips_model(sample["coords"])
                        torch.save(
                            preds,
                            os.path.join(
                                model_save_dir, f"RipsNetPreds_Betti{betti_}.pt"
                            ),
                        )
